...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 7);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 23);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 27);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 38);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 39);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 40);
...
INSERT INTO UserServiceFix.RolePermission (RoleId, PermissionId) VALUES (4, 41);
...
create
    definer = root@`%` procedure CreatePriority(IN _priorityName varchar(50))
BEGIN
    INSERT INTO Priority (PriorityName)
    VALUES (_priorityName);
END;
...
create
    definer = root@`%` procedure CreateTaskStatus(IN _statusName varchar(50))
BEGIN
...
create
    definer = root@`%` procedure CreateUserStoryStatus(IN _statusName varchar(50))
BEGIN
    INSERT INTO UserStoryStatus (StatusName)
    VALUES (_statusName);
END;
...
create
    definer = root@`%` procedure DeletePriority(IN _priorityId int)
BEGIN
    DELETE FROM Priority WHERE PriorityId = _priorityId;
END;
...
create
    definer = root@`%` procedure DeleteTaskStatus(IN _statusId int)
BEGIN
    DELETE FROM TaskStatus WHERE StatusId = _statusId;
END;
...
create
    definer = root@`%` procedure DeleteUserStoryStatus(IN _statusId int)
BEGIN
    DELETE FROM UserStoryStatus WHERE StatusId = _statusId;
END;
...
create
    definer = root@`%` procedure GetPriorityById(IN _priorityId int)
BEGIN
    SELECT PriorityId AS priority_id,
           PriorityName AS priority_name
    FROM Priority WHERE PriorityId = _priorityId;
END;
...
create
    definer = root@`%` procedure GetTaskStatusById(IN _statusId int)
BEGIN
    SELECT * FROM TaskStatus WHERE StatusId = _statusId;
END;
...
create
    definer = root@`%` procedure GetUserById(IN _userId varchar(36))
BEGIN
    SELECT 
        USERID AS user_id,
        ...
        EMAIL AS email,
        CREATEDAT AS created_at
    FROM UserLookup WHERE userId = _userId;
END;
...
create
    definer = root@`%` procedure GetUserStoryStatusById(IN _statusId int)
BEGIN
    SELECT
        StatusId AS status_id,
        StatusName AS status_name
        FROM UserStoryStatus WHERE StatusId = _statusId;
END;
...
create
    definer = root@`%` procedure UpdatePriority(IN _priorityId int, IN _priorityName varchar(50))
BEGIN
    UPDATE Priority
    SET PriorityName = _priorityName
...
create
    definer = root@`%` procedure UpdateSprint(IN p_sprintId int, IN p_sprintName varchar(255), IN p_startDate date,
                                              IN p_endDate date, IN p_status int, IN p_goal text,
                                              IN p_updatedBy varchar(36))
BEGIN
    DECLARE v_oldStatus INT;
    DECLARE v_projectId VARCHAR(36);
    DECLARE v_activeStatusId INT;

    -- Lấy StatusId hiện tại và ProjectId của Sprint đang cập nhật
    SELECT StatusId, ProjectId INTO v_oldStatus, v_projectId
    FROM Sprint
    WHERE SprintId = p_sprintId;

    -- Lấy StatusId của trạng thái 'active'
    SELECT StatusId INTO v_activeStatusId
    FROM SprintStatus
    WHERE StatusName = 'Active';

    -- Nếu người dùng muốn cập nhật sang trạng thái 'active' và trạng thái hiện tại không phải 'active'
    IF p_status = v_activeStatusId AND v_oldStatus <> v_activeStatusId THEN
        -- Kiểm tra xem có Sprint nào khác trong cùng project đang active không
        IF EXISTS (
            SELECT 1 FROM Sprint
            WHERE ProjectId = v_projectId
              AND StatusId = v_activeStatusId
              AND SprintId <> p_sprintId
        ) THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Another sprint is already active in this project.';
        END IF;
    END IF;

    -- Cập nhật Sprint
    UPDATE Sprint
    SET Name = p_sprintName,
        StartDate = p_startDate,
        EndDate = p_endDate,
        StatusId = p_status,
        Goal = p_goal,
...
create
    definer = root@`%` procedure UpdateTaskStatus(IN _statusId int, IN _statusName varchar(50))
BEGIN
    UPDATE TaskStatus
    SET StatusName = _statusName
...
create
    definer = root@`%` procedure UpdateUserStoryStatus(IN _statusId int, IN _statusName varchar(50))
BEGIN
...
(
    ...
    constraint FK_Bug_AssignedTo
        foreign key (AssignedTo) references UserLookup (userId)
            on delete set null,
    constraint FK_Bug_CreatedBy
        foreign key (CreatedBy) references UserLookup (userId)
            on delete set null,
    constraint FK_Bug_ReportedBy
        foreign key (ReportedBy) references UserLookup (userId)
            on delete set null,
    constraint FK_Bug_UpdatedBy
        foreign key (UpdatedBy) references UserLookup (userId)
            on delete set null
);
...
(
    ...
    constraint BugHistory_ibfk_2
        foreign key (StatusId) references BugStatus (StatusId),
    constraint FK_BugHistory_UpdatedBy
        foreign key (UpdatedBy) references UserLookup (userId)
            on delete set null
);
...
(
    ...
    UpdatedAt   datetime                           null,
    updatedBy   varchar(36)                        null,
    constraint FK_ProjectLookup_CreatedBy
        foreign key (createdBy) references UserLookup (userId)
            on delete set null,
    constraint FK_ProjectLookup_UpdatedBy
        foreign key (updatedBy) references UserLookup (userId)
            on delete set null
);
...
(
    ...
    createdAt   datetime default CURRENT_TIMESTAMP null,
    ProjectId   varchar(36)                        null,
    constraint FK_UserStoryLookup_AssignedTo
        foreign key (AssignedTo) references UserLookup (userId)
            on delete set null,
    constraint FK_UserStoryLookup_CreatedBy
        foreign key (createdBy) references UserLookup (userId)
            on delete set null
);
...
(
    ...
    updatedBy      varchar(36)                        null,
    updatedAt      datetime                           null on update CURRENT_TIMESTAMP,
    constraint FK_TaskLookup_AssignedTo
        foreign key (assignedTo) references UserLookup (userId)
            on delete set null,
    constraint FK_TaskLookup_CreatedBy
        foreign key (createdBy) references UserLookup (userId)
            on delete set null,
    constraint FK_TaskLookup_UpdatedBy
        foreign key (updatedBy) references UserLookup (userId)
            on delete set null,
    constraint fk_story
        foreign key (storyId) references UserStoryLookup (storyId)
